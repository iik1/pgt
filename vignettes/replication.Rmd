...
data.frame(farm = fit$results$id, b_star = fit$results$b_star,
           z_star = fit$results$z_star, a_star = fit$results$a_star)
```

The minimal controlled emissions are $(16, 16, 16, 20, 16)$, which match
the paper's Table 2. Because the abatement output is observed, the fit
also reports the projection's implied uncontrolled emission `z_star`
and abatement `a_star`; for farms A, D and E these match the paper's
Table 2 uncontrolled and abatement entries, while for the frontier
farms the optimal peer mix is not unique and the split between
`z_star` and `a_star` is solver-dependent.

## Rødseth (2025), Table 3: the directional representation

The factorially determined multi-output model (Eq. 14, abatement fixed at
...
inefficiency of 6.5: farms A to D reproduce Table 3. Farm E is the one
divergence, shown side by side above. The paper prints
$(\theta_y, \theta_b) = (0.8, 1.0)$; on the printed data the package
returns $(0.789, 0.921)$, the exact optimum of the programme. The
paper's data are printed to one decimal, so the divergence is
conditional on treating them as exact. Common coefficients
$v = 7/6$, $u_{\text{feed}} = 31/25$ and $u_{\text{piglet}} = 349/300$,
with the printed inputs, meat and controlled emissions, give
uncontrolled emissions and abatement that round to the printed columns,
and a fitted Table 3 whose every row, farm E included, rounds to the
printed one:

```{r}
u_c <- c(feed = 31 / 25, piglet = 349 / 300, labor = 0, capital = 0)
x_pig <- as.matrix(pigfarms[, names(u_c)])
z_c <- as.vector(x_pig %*% u_c) - v * pigfarms$meat
a_c <- z_c - pigfarms$controlled
c(uncontrolled = all(round(z_c, 1) == pigfarms$uncontrolled),
  abatement = all(round(a_c, 1) == pigfarms$abatement))
rc <- pgt(pgt_tech(x = x_pig, y = pigfarms$meat, b = pigfarms$controlled,
                   u = u_c, v = v, a = a_c, id = pigfarms$farm),
          model = "fdmo")$results
data.frame(farm = rc$id, gross = round(rc$gross, 3),
           good_eff = round(rc$good_eff, 3),
           bad_eff = round(rc$bad_eff, 3),
           maximal_y = round(rc$maximal_y, 3))
```

These coefficients are one set consistent with the printed data, not
necessarily the paper's own; the farm E row can be confirmed only with
the unrounded data.
...
data.frame(id = rb$id, output_eff = round(rb$output_eff, 4),
           emission_eff = round(rb$efficiency, 4), mean_eff = round(rb$mean_eff, 4))
```

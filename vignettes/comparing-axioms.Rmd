...
the *Journal of Productivity Analysis*, opened by Førsund (2021) and
...
The models split into two families. The materials-balance models are
built on the account $u'x_l - v y_l \ge b_l$, which enters each
programme differently:

- `wgd`: weak-G-disposability (Rødseth 2025), where the account enters
  only as a valuation of retained output content in the objective.
- `mb_cost`: the materials-balance cost model (Coelli et al. 2007),
  where the flow coefficients act as a price system on the inputs,
...
The principal scores are all normalised so that 1 is efficient, but they

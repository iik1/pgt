...
The `technology` argument selects the reference technology. The
default `"wd"` imposes weak disposability of the bad output in the
...
comparator under which the index is defined. The `"input_envelope"` and `"envelope"` options
treat the bad output as reducible to the peer emission envelope and
are exploratory companions; the input-keeping variant matches the
shape of the input-fixed benchmark of `pgt()` rather than its Eq. 6
programme.
...
`pgt_ml()` index. The example takes the sixty plants of one year of
the synthetic `steeldemo` panel, an independent cross-section.
...
model with exponential inefficiency and two known frontiers, the
affine $b_{\min}(y) = 2 + 0.5y$ and the curved
$b_{\min}(y) = 1 + 0.05y^2$, where the true score
$\theta = b_{\min}(y)/b$ is the estimand. Two hundred replicates per
cell, `B = 100`, nominal level 95 per cent. A unit gets no interval
when fewer than `B/2` of its subsample programmes are feasible, so the
study reports availability, coverage among units with an interval
("given interval") and the share of all units whose interval exists
and covers ("delivered"). The table shows the cells at the defaults
$m = \mathrm{round}(L^{0.7})$ and $\kappa = 2/3$ from the precomputed
results shipped with the package.

```{r, echo = FALSE}
cov <- read.csv(system.file("simulations", "coverage-results.csv",
                            package = "pgt"))
dflt <- abs(cov$kappa - 2 / 3) < 1e-9 & cov$m == round(cov$L^0.7)
knitr::kable(
  cov[dflt, c("dgp", "L", "available", "coverage_cond",
              "coverage_delivered", "coverage_frontier",
              "coverage_interior", "coverage_group_means")],
  digits = 3, row.names = FALSE,
  col.names = c("frontier", "L", "available", "given interval",
                "delivered", "frontier (theta > 0.9)", "interior",
                "group means")
)
```

On the affine frontier the intervals over-cover, and on the curved one
they under-cover by two to three points at both sample sizes. The
missing intervals fall on the units with the largest output. The
shipped results also vary `m` and `kappa` at `L = 100`: a larger
subsample or a faster rate narrows the intervals and lowers coverage,
to 0.77 under `kappa = 1` on the curved frontier. One simulation
design is evidence, not proof: the DEA rate remains borrowed for the
materials-balance programmes, which is why the intervals keep their
heuristic label and why `boot_pgt_sensitivity()` belongs in the
workflow.
